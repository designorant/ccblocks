# Agent setup for `ccblocks`

This is a deepsec scanning workspace. Project `ccblocks` was just registered
(target: `..`). Setup is incomplete — `data/ccblocks/INFO.md`
...
2. **Fill in `data/ccblocks/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id ccblocks
pnpm deepsec process --project-id ccblocks
```
